...
class fio_perf_tester()  :
    ...
    def test_rand_write_increasing_thread(self, qd, bs, fs, time, num_test) :
        self.__make_and_set_log_name("RW", s_bs=bs, e_bs=bs, s_thread=1, e_thread=num_test, s_qd=qd, e_qd=qd, fsize=fs)
        t = 1
        for i in range(num_test):
            self.rand_write(t = t, qd = qd, bs = bs, fs = fs, time = time)
            t = t * 2
    ...
    def test_seq_read_increasing_thread(self, qd, bs, fs, num_test) :
        ...
        t = 1
        for i in range(num_test):
            self.seq_read(off='0M', qd=qd, t=t, bs=bs, fs=fs)
            t = t * 2

    def test_rand_read_qd1_increasing_bs(self, fs, time, num_test) :
        bs = 4096*4
        self.__make_and_set_log_name("RR", s_bs=str(bs), e_bs=2**num_test, s_thread=1, e_thread=1, s_qd=1, e_qd=1, fsize=fs)
...
if __name__ == "__main__" :
    ...
    #tester.seq_write(off='0M', t=1, qd=1, bs='512K', fs=fs)

    #tester.rand_write(t=1,qd=1,bs='4K', fs=fs, time=3600)
    #tester.test_seq_write_increasing_bs(qd=1, t=32, fs=fs, num_test=num_test)

    tester.test_rand_write_increasing_thread(qd=1, bs='4K', fs=fs, time = t, num_test=1)
    tester.test_rand_read_increasing_thread(qd=1, bs='4K', fs=fs, time = t, num_test=1)

    tester.test_rand_write_increasing_thread(qd=1, bs='128K', fs=fs, time = t, num_test=num_test)
    tester.test_rand_read_increasing_thread(qd=1, bs='128K', fs=fs, time = t, num_test=num_test)

    #tester.test_rand_read_qd1_increasing_bs(fs=fs, time=t, num_test=16)
    #tester.rand_read(t = 256, qd = 1, bs = str(4096), fs = fs, time = t)
    #tester.rand_read(t = 256, qd = 1, bs = str(8192), fs = fs, time = t)


    #tester.test_rand_read_increasing_thread(qd=1, bs='8K', fs=fs, time=t, num_test=9)

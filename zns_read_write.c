...
bool __zns_write(struct zns_ftl *zns_ftl, struct nvme_request * req, struct nvme_result * ret)
{
	...
	nsecs_latest = ssd_advance_write_buffer(&zns_ftl->ssd,
	...
	for (lpn = slpn; lpn <= elpn; lpn+=pgs) {
		...
		if ((pg_off + pgs) == spp->pgs_per_oneshotpg) {	
			...
			swr.ppa = &ppa;
			nsecs_completed = ssd_advance_nand((struct ssd*)zns_ftl, &swr);
			...
			enqueue_writeback_io_req(req->sq_id, nsecs_completed, zns_ftl->write_buffer, spp->pgs_per_oneshotpg * spp->pgsz);
		} 
	}
...
bool __zns_write_zrwa(struct zns_ftl *zns_ftl, struct nvme_request * req, struct nvme_result * ret)
{
	...
	NVMEV_DEBUG("%s slba 0x%llx nr_lba 0x%llx zone_id %d state %d wp 0x%llx zrwa_impl_start 0x%llx zrwa_impl_end 0x%llx  buffer %d\n",
														__FUNCTION__, slba, nr_lba, zid, state, prev_wp, zrwa_impl_start, zrwa_impl_end, zns_ftl->zwra_buffer[zid].remaining);
	...
	if (elba >= zrwa_impl_start) {
		...
		NVMEV_DEBUG("%s implicitly flush zid %d wp before 0x%llx after 0x%llx buffer %d",
														__FUNCTION__, zid,  prev_wp, zone_descs[zid].wp + nr_lbas_flush, zns_ftl->zwra_buffer[zid].remaining);
	...
	while (remaining > 0) {
		...
		if ((pg_off + pgs) == spp->pgs_per_oneshotpg) {	
			...
			enqueue_writeback_io_req(req->sq_id, nsecs_completed, &zns_ftl->zwra_buffer[zid], spp->pgs_per_oneshotpg * spp->pgsz);
		} 
...
bool zns_write(struct nvme_request * req, struct nvme_result * ret)
{
	...
	if (buffer_allocate(zns_ftl->write_buffer, LBA_TO_BYTE(nr_lba)) < LBA_TO_BYTE(nr_lba))
			return false;
